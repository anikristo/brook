...
TEST(SubtractBroadcast, Infer){

    // Read input for test files
    std::string tf_in_file = "test/unit_test/subtract_broadcast/tf.in";
    std::string tf_out_file = "test/unit_test/subtract_broadcast/tf.out";
    ...
    if(nr_tests == tf_out.size()){
        ...
        for(auto idx = 0; idx < nr_tests; ++idx){
            ...
            accu &= y_true.isApprox(y_pred, 1e-5); // TODO Get precision from args
        }
...
int main(int argc, char **argv) {
  ...
  ::testing::GTEST_FLAG(filter) = "SubtractBroadcast.*";
